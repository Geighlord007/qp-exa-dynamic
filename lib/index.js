...
/**
 * The model-facing tool this plugin adds beside `web_search`.
 *
 * It exists for one reason: `ctx.web.search()` caps its result at the CALLER's
 * `request.maxResults`, and `dsh-tool-web` sends its own `searchMaxResults` on
 * every call — so no provider can ever exceed that cap, and raising it means
 * forking an agent preset. This tool owns its own `request.maxResults`, so the
 * model can ask for more sources in a single call without the deployment's
 * preset being touched, and without a copy that stops tracking upstream.
 *
 * The name is deliberately distinct from `web_search`: two tools under one name
 * would collide in the registry.
 */
const TOOL_NAME = "exa_search";
/**
 * Hard ceiling on this tool's `maxResults`, independent of `dsh-tool-web`'s.
 * Exa accepts up to 100; this bound is ours, and it exists so one call cannot
 * flood a context window (dynamic highlights measured ~1.6k characters each).
 */
const TOOL_MAX_RESULTS_LIMIT = 50;
/** Safety notice prepended to every result, matching `dsh-tool-web`'s wording. */
...
const USER_AGENT = "qp-exa-dynamic/0.1.0";
 ...
 */
const SETTINGS_NAMESPACE = "qp-exa-dynamic";
...
/**
 * Clamp the model's `maxResults` argument for {@link TOOL_NAME}.
 *
 * This tool owns `request.maxResults` for its own calls — that is the whole
 * point of it — so it also owns the bound. Non-numbers and nonsense fall back
 * to the configured default rather than failing the call.
 *
 * @param value - the model's argument, if it sent one.
 * @returns the limit to send, or `undefined` to use the configured default.
 */
function clampToolMaxResults(value) {
	if (typeof value !== "number" || !Number.isFinite(value)) return undefined;
	const whole = Math.floor(value);
	if (whole < 1) return 1;
	return Math.min(whole, TOOL_MAX_RESULTS_LIMIT);
}

/** A source's display label: its title, else its hostname, else the raw URL. */
...
/**
 * Format a result as one model-facing text block, matching `dsh-tool-web`'s
 * shape so both search tools read the same way to a model.
 *
 * @param sources - the normalized sources, in result order.
 * @returns the text the model receives.
 */
function formatToolSources(sources) {
	...
	if (sources.length === 0) {
		...
		const lines = sources.map((source) => {
			...
			return `- [${sourceLabel(source.url, source.title)}](${source.url})${suffix}`;
		});
		parts.push(`Sources:\n${lines.join("\n")}`);
	}
...
function apply(ctx, config) {
	...
	ctx.inject(["commands"], (commandCtx) => {
		try {
			commandCtx.commands.register({
				...
				// `input` MUST be declared, hint included. The composer's Enter
				// decision table claims a parameterised line only when the command
				// declares `input` (dsh-client-ui-commands/lib/client.js:747); with
				// it absent, `/exa status` and every other argument form falls
				// through to `if (!bare) return void 0` on line 751 and is sent to
				// the model as an ordinary chat message instead of the handler.
				//
				// Kept short and free of brackets: the composer surfaces this as an
				// editable template, so a bracketed one invited bracketed input.
				// `interpretExaCommand` strips brackets and quotes anyway, and a
				// surviving `|` means the template came through untouched, which it
				// reports rather than guessing.
				input: { hint: "on|off|status|type deep|results 3" },
	...
	// A second, model-facing search tool. The seam caps every search at the
	// caller's `request.maxResults`, and `dsh-tool-web` sends its own
	// `searchMaxResults` on every call — which is why no provider can exceed it
	// and why changing that cap means forking an agent preset. THIS tool owns its
	// own `request.maxResults`, so the count becomes a per-call model argument
	// instead of a deployment-level ceiling, with no preset fork and no copy that
	// stops tracking upstream.
	//
	// Both registries are optional for the provider, so an absent tool surface
	// degrades the plugin rather than taking search down with it.
	ctx.inject(["tools", "systemPrompt"], (toolCtx) => {
		try {
			toolCtx.tools.register(
				defineTool({
					name: TOOL_NAME,
					description: `Search the web and choose how many sources come back. Same index and provider as web_search, but accepts maxResults (1-${TOOL_MAX_RESULTS_LIMIT}) for questions that need broader coverage than the default result count.`,
					parameters: {
						query: { type: "string", required: true, description: "The search query." },
						maxResults: {
							type: "integer",
							description: `How many sources to return, 1-${TOOL_MAX_RESULTS_LIMIT}. Omit for the deployment default.`,
						},
					},
					output: {
						schema: {
						...
						render: (_args, value) => [{ type: "text", text: formatToolSources(value.sources) }],
					},
					isConcurrencySafe: () => true,
					async execute(args, exec) {
						const input = args ?? {};
						const query = typeof input.query === "string" ? input.query : "";
						const limit = clampToolMaxResults(input.maxResults);
						const result = await ctx.web.search(
							limit === undefined ? { query } : { query, maxResults: limit },
							exec.signal,
						);
						return { sources: result.sources, truncated: result.truncated };
					},
				}),
			);
		} catch {
			// Losing the tool is a smaller loss than losing search.
		}

		// Guidance beside `web_search`'s own section (TOOL_WEB_SEARCH is 2000, the
		// fetch section 2100), so a model reads when to reach for this one.
		try {
			toolCtx.systemPrompt.section({
				name: `tool:${TOOL_NAME}`,
				order: toolCtx.systemPrompt.getSectionOrder("TOOL_WEB_SEARCH") + 10,
				text: ({ scope }) =>
					toolCtx.tools.get(TOOL_NAME, scope) === undefined
						? ""
						: `The ${TOOL_NAME} tool searches the same provider as web_search but takes a maxResults argument (up to ${TOOL_MAX_RESULTS_LIMIT}), so a question needing broad coverage can be answered in one call instead of several. Prefer web_search for ordinary lookups; reach for ${TOOL_NAME} when the default number of sources would be too thin.`,
			});
		} catch {
			// No prompt section is a smaller loss than no tool.
		}
	});
}
